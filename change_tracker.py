# -*- coding: utf-8 -*-
"""
Created on Fri Mar  8 11:49:42 2019
...
for i in range(200):
    ...
    if(z==5):
        for j in range(frames):
            ...
            snake_motion[i,:,:,j] = create_circular_mask(img_size,img_size, centre = [0,0], radius = 0)
...
X_test_cont = np.zeros((img_size,img_size,200*16,1))
...
new_model = load_model("C:/Users/cks/Documents/practice codes/dot_3d_cnn_models/model_6_class.h5")
...
motion_list = ["Down ↓","Right →","Diagonal ↘","Up ↑","Left ←","No Motion"]
...
prediction_matrix = np.zeros(((3200-16),1))
...
for i in range(3200):
    ...
    prediction_matrix[i] = temp_class
...
### Frame by Frame motion of the looping square along with the prediction (sliding window)
    ...
    if(i == 0):
        ...
        if(prediction_matrix[i] != prediction_matrix[i-1]):
            print("\n")
            print("From     ",motion_list[int(prediction_matrix[i-1][0])],"     To   ",motion_list[int(prediction_matrix[i][0])])
            
            if(prediction_matrix[i-1][0] == 1 and prediction_matrix[i] == 4):
                if(door_status == 0):
                    print("\n")
                    print("Command Received : Alohomora! Open the Doors! ")
                    print("\n")
                    print("|##           ##|")
                    print("|##           ##|")       
                    print("|##           ##|")
                    ...
                    print("\n")
                    print("Command Received : Alohomora! The door is already Open! ")
                    print("\n")
                    
            if(prediction_matrix[i-1][0] == 0 and prediction_matrix[i] == 3):
                if(door_status == 1):
                    print("\n")
                    print("Command Received : Closing the Doors! ")
                    print("\n")
                    print("|######|######|")
                    print("|######|######|")     
                    print("|######|######|")
                    ...
                    print("\n")
                    print("Command Received : The door is already Closed! ")
                    print("\n")
                    
                    
            

### Actual data continuous flow visualization
for i in range(200):
