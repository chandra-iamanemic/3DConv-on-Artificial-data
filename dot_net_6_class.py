# -*- coding: utf-8 -*-
"""
Created on Thu Mar  7 14:05:45 2019
...
motion_6 = np.zeros((img_size*img_size,img_size,img_size,frames))
...
for i in range(img_size):
    for j  in range(img_size):
        ...
        for k in range(frames):
            ...
            e = create_circular_mask(img_size,img_size, centre = [0, 0], radius=0)
...
#Storing motion_6 to X
test_count = 0
for i in range(img_size*img_size):
    
    
    if(test_count%20==0):
        X_test.append(motion_6[i,:,:,:])
        Y_test.append(5)
    else:
        X.append(motion_6[i,:,:,:])
        labels.append(5)
...
Y_labels = to_categorical(Y_train, num_classes=6)
...
history = model.fit(X_train,Y_train,batch_size = 4, epochs=1, validation_split = 0.1, shuffle = 'True')


model.save("C:/Users/cks/Documents/practice codes/dot_3d_cnn_models/model_6_class.h5")
